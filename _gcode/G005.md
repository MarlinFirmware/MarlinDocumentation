...
notes:
  ...
  - the first control point is the current position of the head. X/Y is the destination and therefore the last control point of the spline.
  - control point 1 is calculated by taking the head's current position +I/J and control point 2 is calculated from the head's current position +P/Q
  - I/J can be omitted, which results in these offsets being zero, this results in a 3-point spline (try this in the interactive demo below), however P and Q are neccessary (otherwise it just becomes a linear movement)
