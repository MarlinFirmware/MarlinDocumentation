...
examples:
  ...
  -
    ...
    code:
      - G5 P0 Q-3 X2 Y2
---
[`G5`](/docs/gcode/G005.html) creates a cubic B-spline in the XY plane with the X and Y axes only. `P` and `Q` parameters are required. `I` and `J` are required for the first [`G5`](/docs/gcode/G005.html) command in a series. For subsequent [`G5`](/docs/gcode/G005.html) commands, either both `I` and `J` must be specified, or neither. If `I` and `J` are unspecified, the starting direction of the cubic will automatically match the ending direction of the previous cubic (as if `I` and `J` are the negation of the previous `P` and `Q`).
