...
- tag: F
  optional: true
  description: The maximum movement rate of the move between the start and end point. The feedrate set here applies to subsequent moves that omit this parameter. As with other `rate` parameters this value is specified in current units per minute.
  values:
  - tag: rate
    type: float
