...
The Probe Target commands are used to probe towards (or away from) a workpiece to determine its precise position. You might, for example, use a grounded metal workpiece, with a metal probe spliced into the bed probe circuit. If you use a bed probe that is set up as an additional dummy extruder, the following steps are required:
- hotend offsets for the probing tool should be equal to the nozzle-to-probe offset.
- Perform a tool change to the probing tool.
- Use [`M401`](/docs/gcode/M401.html) and [`M402`](/docs/gcode/M402.html) to deploy and stow the probe.
