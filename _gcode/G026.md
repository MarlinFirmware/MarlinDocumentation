...
experimental: true
requires: AUTO_BED_LEVELING_UBL,UBL_G26_MESH_VALIDATION
group: calibration
...
long:
  - |
    `G26` Mesh Validation Pattern is designed to be used in conjunction with mesh-based leveling to test the accuracy of the probed mesh. This feature is currently only implemented for [Unified Bed Leveling](/docs/features/unified_bed_leveling.html), so `AUTO_BED_LEVELING_UBL` is required along with `UBL_G26_MESH_VALIDATION` in `Configuration.h`
