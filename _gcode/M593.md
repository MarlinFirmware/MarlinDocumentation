...
- Use a slicer that provides custom G-code macros for layer change. For example you can open [Kiri:Moto](//grid.space/kiri/) or download [Prusa Slicer](//www.prusa3d.com/page/prusaslicer_424). Cura will require you to install the Post Processing Plugin from the Cura Marketplace and use a custom script.
...
- In Cura, you'll first need to add the .py file in [this repository](//www.github.com/dsdanielko/cura-ringing-tower-script/) to your cura scripts folder. Then in Cura you'll need enable **Spiralize Outer Contour**. Then go to **Extensions > Post Processing > Modify G-Code**. Click **Add a script** in the window that opens, then use the dropdown menu to find **Input Shaping**. Use with default settings to run a test range of 15Hz to 60Hz.
