...
Nozzle and Bed Temperature in Marlin is commonly controlled with a PID algorithm. PID operates as a feedback loop with Proportional-Integral-Derivative control terms. Functionally the further the Measured-Temperature (process variable) is from the Set-Temperature (setpoint), the more correction is applied to the power output. If you could supply PID with perfect Measured-Temperature information, in a predictable steady state, it could, in principle, apply perfect control. However Measured-Temperature information comes from sensors which exhibit both latency and noise. In addition PID control can struggle to respond to rapid changes.  This can lead to overshoot and oscillation in the Actual-Temperature. 

Model predictive control (MPC) takes a different approach to Temperature control from PID. Instead of trying to control against the Measured-Temperature, it maintains a simulation of the system and uses the simulated hotend temperature to plan an optimal power output. The simulation has no noise and no latency, making near perfect control possible. This has the added benefit of being able to compensate directly for extrusion speed and part cooling. To prevent the simulated system state diverging from the real life hotend state, the simulated temperature is continually gently dragged towards the temperature measure from the sensor. This does introduce a little noise and latency into the simulated system but the effect is far smaller than for PID.
